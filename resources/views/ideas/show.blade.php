<x-app-layout>
    <div>
        <a href="{{ route('ideas.index') }}" class="text-mint hover:text-deep-blue text-sm font-medium inline-flex items-center">
    ...
    <div class="glass-card rounded-2xl p-6 md:p-8 animate-fade-in-up">
        ...
        <!-- Действия преподавателя / администратора -->
        @auth
            @if((Auth::user()->isTeacher() || Auth::user()->isAdmin()) && in_array($idea->status, ['published', 'approved', 'rejected']))
                <div class="mt-6 flex items-center space-x-3">
                    @if($idea->status !== 'approved')
                        <form method="POST" action="{{ route('ideas.status.update', $idea) }}">
                            ...
                            <button type="submit" class="px-4 py-2 bg-mint text-white font-medium rounded-xl hover:bg-mint/90 transition shadow-sm text-sm">
                                Одобрить
                            </button>
                        </form>
                    @endif
                    @if($idea->status !== 'rejected')
                        <form method="POST" action="{{ route('ideas.status.update', $idea) }}">
                            ...
                            <button type="submit" class="px-4 py-2 bg-alert-coral text-white font-medium rounded-xl hover:bg-red-500 transition shadow-sm text-sm">
                                Отклонить
                            </button>
                        </form>
                    @endif
                ...
                @if($idea->status === 'approved' && !$idea->project)
                    <div class="mt-4">
                        <a href="{{ route('projects.create', ['idea_id' => $idea->id]) }}" class="px-4 py-2 bg-deep-blue text-white font-medium rounded-xl hover:bg-opacity-90 text-sm">
        ...
        <div class="mt-8 pt-6 border-t border-gray-100 flex items-center">
            @if($idea->user->avatar)
                <a href="{{ route('profile.show', $idea->user) }}">
                    <img src="{{ Storage::url($idea->user->avatar) }}" class="w-10 h-10 rounded-full object-cover">
                </a>
            @else
                <a href="{{ route('profile.show', $idea->user) }}" class="w-10 h-10 rounded-full bg-mint flex items-center justify-center text-white font-semibold">
                    {{ Str::substr($idea->user->name, 0, 1) }}
                </a>
            @endif
            <div class="ml-3">
                <a href="{{ route('profile.show', $idea->user) }}" class="text-sm font-medium text-deep-blue hover:text-mint transition">
                    {{ $idea->user->name }}
                </a>
                <p class="text-xs text-slate-light">
                    {{ $idea->user->role === 'teacher' ? 'Преподаватель' : ($idea->user->role === 'admin' ? 'Администратор' : 'Студент') }}
                </p>
            </div>
        </div>
    </div>
