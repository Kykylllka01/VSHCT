<x-app-layout>
  <div class="space-y-8 animate-fade-in-up">
    ...
    <div class="flex items-center space-x-4">
      ...
      @else
        <div
          class="w-16 h-16 rounded-full bg-gradient-to-br from-deep-blue to-mint flex items-center justify-center text-white text-2xl font-bold">
      ...
      @if(Auth::id() === $user->id)
        <a href="{{ route('profile.edit') }}"
          class="ml-auto px-4 py-2 text-sm bg-mint text-white rounded-xl hover:bg-mint/90 transition">
          Редактировать
        </a>
      @endif
    </div>
    ...
    <div class="glass-card rounded-2xl p-6">
      <h2 class="text-xl font-semibold text-deep-blue mb-4">Статистика проектов</h2>
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
        <div class="p-3 bg-white/50 rounded-xl">
          <p class="text-2xl font-bold text-deep-blue">{{ $totalProjects }}</p>
          <p class="text-sm text-slate-light">Всего проектов</p>
        </div>
        <div class="p-3 bg-white/50 rounded-xl">
          <p class="text-2xl font-bold text-deep-blue">{{ $ledProjectCount }}</p>
          <p class="text-sm text-slate-light">Руководитель</p>
        </div>
        <div class="p-3 bg-white/50 rounded-xl">
          <p class="text-2xl font-bold text-deep-blue">{{ $projectCount }}</p>
          <p class="text-sm text-slate-light">Участник</p>
        </div>
      </div>
    </div>
        ...
        <div class="space-y-3">
          @foreach($user->ideas as $idea)
            <a href="{{ route('ideas.show', $idea) }}"
              class="block p-3 bg-white/50 rounded-xl hover:bg-white/80 transition">
              <span class="font-medium text-deep-blue">{{ $idea->title }}</span>
              <span
                class="text-xs text-slate-light ml-2">{{ $idea->status === 'published' ? 'Опубликовано' : ($idea->status === 'approved' ? 'Одобрено' : $idea->status) }}</span>
            </a>
          @endforeach
        </div>
      @else
